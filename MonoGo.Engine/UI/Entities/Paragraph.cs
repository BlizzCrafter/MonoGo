                ...
                foreach (var command in commandParts)
                {
                    ...
                    try
                    {
                        switch (key)
                        {
                            ...
                            case "FC":
                                currCommand.FillColor = ColorHelper.HexToColor(value!);
                                expectedValuesCount = 1;
                                break;
            ...
            if (TextOverflowMode != TextOverflowMode.Overflow)
            {
                ...
                while (lineIndex < ret.Count)
                {
                    ...
                    if (line.Length > 1)
                    {
                        ...
                        void BreakLineIfNeeded(int lineWidth, int maxWidth, TextOverflowMode mode)
                        {
                            if (lineWidth > maxWidth)
                            {
                                ...
                                while (keepSearching && (lastSpaceIndex > 0))
                                {
                                    ...
                                    var cutLineWidth = Renderer.MeasureText(cutLine, font, fontSize, spacing).X;
                        ...
                        var lineWidth = Renderer.MeasureText(line, font, fontSize, spacing).X;
        ...
        protected override void DrawEntityType(ref Rectangle boundingRect, ref Rectangle internalBoundingRect, DrawMethodResult parentDrawResult, DrawMethodResult? siblingDrawResult)
        {
            ...
            if (!string.IsNullOrEmpty(Text))
            {
                    ...
                    (_cachedTextFontId != font))
                {
                    ...
                    _lineHeight = Renderer.GetTextLineHeight(font, fontSize);
                    _textHeight = _lineHeight * _cachedProcessedText.Length;
                    _textWidth = 0;
                    foreach (var line in _cachedProcessedText)
                    {
                        var lineWidth = Renderer.MeasureText(line.Line, font, fontSize, spacing).X;
                ...
                void DrawStateText(EntityState state, Rectangle internalBoundingRect, float alpha)
                {
                    ...
                    // measure space width
                    var spaceWidth = (int)(Renderer.MeasureText("_", font, fontSize, 1f).X * 0.5f);
                    ...
                    foreach (var line in _cachedProcessedText)
                    {
                        ...
                        var lineSize = Renderer.MeasureText(line.Line, font, fontSize, spacing);
                        ...
                        if (styleCommands == null || styleCommands.Count == 0)
                        {
                            if (_beforeDrawingLineNoStyleCommands != null)
                            {
                                ...
                                {
                                    Renderer.DrawText(effectId, processedLine, font, fontSize, position, currTextStyle.FillColor ?? fillColor, currTextStyle.OutlineColor ?? outlineColor, currTextStyle.OutlineWidth ?? outlineWidth, spacing);
                                }
                            }
                            else
                            {
                                Renderer.DrawText(effectId, line.Line, font, fontSize, position, currTextStyle.FillColor ?? fillColor, currTextStyle.OutlineColor ?? outlineColor, currTextStyle.OutlineWidth ?? outlineWidth, spacing);
                            }
                        }
