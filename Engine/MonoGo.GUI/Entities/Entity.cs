...
namespace MonoGo.GUI.Entities
{
    /// <summary>
    /// MonoGo.GUI.Entities contains all the UI elements you can create and use in your layouts.
    /// </summary>
    ...
    public abstract class EntityUI
    {
        ...
        static EntityUI()
        {
            EntityUI.MakeSerializable(typeof(EntityUI));
        }

        /// <summary>
        /// List of child elements.
        /// </summary>
        protected internal List<EntityUI> _children = new List<EntityUI>();

        /// <summary>
        /// Get / set children list.
        /// </summary>
        public IReadOnlyList<EntityUI> Children
        {
            get
            {
        ...
        private List<EntityUI> _sortedChildren;
        ...
        protected EntityUI _parent = null;
        ...
        private EntityUI _background = null;
        ...
        public T Find<T> (string identifier, bool recursive = false) where T : EntityUI
        {
            // should we return any entity type?
            bool anyType = typeof(T) == typeof(EntityUI);

            // iterate children
            foreach (EntityUI child in _children)
            {
        ...
        public EntityUI Find(string identifier, bool recursive = false)
        {
            return Find<EntityUI>(identifier, recursive);
        }
        ...
        public bool IsDeepChildOf(EntityUI other)
        {
            // iterate over parents until root, starting with self.
        ...
        protected virtual void DrawChildren(SpriteBatch spriteBatch)
        {
            ...
            List<EntityUI> childrenSorted = GetSortedChildren();

            // draw all children
            foreach (EntityUI child in childrenSorted)
            {
        ...
        public Dictionary<string, EntityUI> ToEntitiesDictionary()
        {
            Dictionary<string, EntityUI> ret = new Dictionary<string, EntityUI>();
            PopulateDict(ret);
            return ret;
        }
        ...
        private void PopulateDict(Dictionary<string, EntityUI> dict)
        {
        ...
        public EntityUI AddChild(EntityUI child, bool inheritParentState = false, int index = -1)
        {
        ...
        public void RemoveChild(EntityUI child)
        {
            // make sure don't already have a parent
            ...
            int index = 0;
            foreach (EntityUI itrChild in _children)
            {
        ...
        virtual public Rectangle CalcDestRect()
        {
            ...
            if ((anchor == Anchor.Auto || anchor == Anchor.AutoInline || anchor == Anchor.AutoCenter || anchor == Anchor.AutoInlineNoBreak))
            {
                ...
                EntityUI prevEntity = GetPreviousEntity(true);
        ...
        public virtual void PropagateEventsTo(EntityUI other)
        {
            OnMouseDown += (EntityUI entity) => { other.OnMouseDown?.Invoke(other); };
            OnRightMouseDown += (EntityUI entity) => { other.OnRightMouseDown?.Invoke(other); };
            OnMouseReleased += (EntityUI entity) => { other.OnMouseReleased?.Invoke(other); };
            WhileMouseDown += (EntityUI entity) => { other.WhileMouseDown?.Invoke(other); };
            WhileRightMouseDown += (EntityUI entity) => { other.WhileRightMouseDown?.Invoke(other); };
            WhileMouseHover += (EntityUI entity) => { other.WhileMouseHover?.Invoke(other); };
            WhileMouseHoverOrDown += (EntityUI entity) => { other.WhileMouseHoverOrDown?.Invoke(other); };
            OnRightClick += (EntityUI entity) => { other.OnRightClick?.Invoke(other); };
            OnClick += (EntityUI entity) => { other.OnClick?.Invoke(other); };
            OnValueChange += (EntityUI entity) => { other.OnValueChange?.Invoke(other); };
            OnMouseEnter += (EntityUI entity) => { other.OnMouseEnter?.Invoke(other); };
            OnMouseLeave += (EntityUI entity) => { other.OnMouseLeave?.Invoke(other); };
            OnMouseWheelScroll += (EntityUI entity) => { other.OnMouseWheelScroll?.Invoke(other); };
            OnStartDrag += (EntityUI entity) => { other.OnStartDrag?.Invoke(other); };
            OnStopDrag += (EntityUI entity) => { other.OnStopDrag?.Invoke(other); };
            WhileDragging += (EntityUI entity) => { other.WhileDragging?.Invoke(other); };
            BeforeDraw += (EntityUI entity) => { other.BeforeDraw?.Invoke(other); };
            AfterDraw += (EntityUI entity) => { other.AfterDraw?.Invoke(other); };
            BeforeUpdate += (EntityUI entity) => { other.BeforeUpdate?.Invoke(other); };
            AfterUpdate += (EntityUI entity) => { other.AfterUpdate?.Invoke(other); };
        }
        ...
        protected EntityUI GetPreviousEntity(bool skipInvisibles = false)
        {
            // no parent? skip
            if (_parent == null) { return null; }

            // get siblings and iterate them
            List<EntityUI> siblings = _parent._children;
            EntityUI prev = null;
            foreach (EntityUI sibling in siblings)
            {
        ...
        virtual protected void UpdateChildren(ref EntityUI targetEntity, ref EntityUI dragTargetEntity, ref bool wasEventHandled, Point scrollVal)
        {
        ...
        /// <param name="wasEventHandled">Set to true if current event was already handled by a deeper child.</param>
        /// <param name="scrollVal">Combined scrolling value (panels with scrollbar etc) of all parents.</param>
        virtual public void Update(ref EntityUI targetEntity, ref EntityUI dragTargetEntity, ref bool wasEventHandled, Point scrollVal)
        {
            // set last scroll var
